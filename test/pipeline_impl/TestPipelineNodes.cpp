...
int main()
{
  ...
  std::shared_ptr<Node> accumulator_node = std::make_shared<AccumulatorNode>(4096);
  std::shared_ptr<Node> fft_node = std::make_shared<FFTNode>(4096);
  ...
  std::dynamic_pointer_cast<FileRecorderNode>(file_recorder_node)->Initialize("out.audio.bin.1");

  audio_player_node->RegisterNodeAsNext(accumulator_node);
  accumulator_node->RegisterNodeAsNext(fft_node);
